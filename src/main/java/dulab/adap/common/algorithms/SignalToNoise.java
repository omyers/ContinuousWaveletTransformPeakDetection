...
//         writeArray(target);
     ...
     *
     * Estimation is equivalent to the default quantile estimate in R:
     * if x[j] and x[j+1] are two values adjacent to the quantile, than
     *         Q[p] = (gamma - 1) * x[j] + gamma * x[j+1]
     * where gamma = (size - 1) * p + 1 - j and j starts from 1
     *
    ...
    public static double filterBySNRandWindowSelect(double[] intensities,int peakLeft,int peakRight)
    {
        ...
        // I think we should use the mean instead of the min. This assumes that the majority of the stuff within our max window
        // is noise.
